...
function render(source, destination) {
  const staticFilePath = `${process.cwd()}/static/mermaid-svgs`;
  const unique = crypto.createHmac('sha1', PLUGIN_NAME).update(source).digest('hex');
  const mmdcExecutable = which.sync('mmdc');
  const mmdPath = path.join(staticFilePath, `${unique}.mmd`);
  const svgFilename = `${unique}.svg`;
  const svgPath = path.join(staticFilePath, svgFilename);
  ...
  return { source, imgSrc: `/mermaid-svgs/${svgFilename}`};
}
