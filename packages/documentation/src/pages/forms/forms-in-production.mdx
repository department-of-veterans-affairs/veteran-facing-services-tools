---
title: Migrating form data
tags: updating schema, migrations
---

# Migrating form data

Once your form is in production and you're also using the [Save in Progress](/forms/save-in-progress) functionality, you have a dilemma when you want to change fields or validation on your form. Since some forms are saved, if you change the structure of the form, the saved data may no longer match, which might cause confusing validation errors for a user. Similarly, if validation changes, a user might have already gone past that field and won't know how to fix the new error.
...
If the current return URL (which is the URL a user will return to when they load a saved form) matches the outdated one, we update it. The `formData` is not changed in this case.
...
This example moves an incorrectly located `isSpanishHispanicLatino` flag and sets it in the right spot.
