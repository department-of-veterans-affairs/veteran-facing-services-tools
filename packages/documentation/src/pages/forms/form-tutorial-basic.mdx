...
This doc walks you through creating a new form and making some simple changes.

Make sure you meet the following prerequisites:

- VA.gov is [running locally](/getting-started).
- [VA.gov Yeoman generator](/platform/tools/generator) is installed locally (first step in the linked page).

If you both of those steps completed, you're ready to get started.

## Create a new form with the Yeoman generator

To begin, create a new form by running the generator from the root of your local `vets-website` repository:
...
Start the site locally (or restart the site if it is already running):
...
Now that you're up and running, you can add a new page and field to the form. If you open `src/applications/new-form/config.js`, you should see a `formConfig` variable:
...
A lot of information is automatically included in the file. Check out the [form config specification](/forms/config-options) to see what each property means. For now, let’s look at the content of the form, which lives in `chapters`.

At the most basic level, our forms consist of: widgets, fields, pages, and chapters.

- **Widgets**: The basic form controls, such as `<input/>` and `<select/>` elements.
- **Fields**: The next level up from widgets, fields contain a widget and a `<label/>`, plus some extra optional description information.
- **Pages**: Collections of fields.
- **Chapters**: Collections of pages.

There's already one chapter in the config file. It contains one page, called `page1`. For now, you can ignore most of the information in the `page1` object. The important properties are `uiSchema` and `schema`.

The `schema` is the initial structure of our page, in the form of a [JSON Schema](https://spacetelescope.github.io/understanding-json-schema/). This property describes the type of data that will result from a user filling in the form. It's also used by the form library to determine what fields and widgets to display in the application, except when overridden by `uiSchema`. 

The `uiSchema` is used to add more customization to the form's look and feel. To customize object fields in the uiSchema, the structure of the uiSchema should be `{key: value}`, where `key` is the property key and `value` is an object with the uiSchema configuration for that particular property. 
...
Now if you go to http://localhost:3001/new-form/first-page you should see this field:

![](../../images/forms/first-field.png)

You can add a title to the field by adding to `uiSchema`:
...
If you do fill in the required information and click **Continue**, you'll end up on the review page. The review page is generated based on your chapters and pages and provides a quick way to review the data that you've entered:
...
That's it! Continue on in our documentation to [learn about building more complex forms](/forms/form-tutorial-intermediate) and the whole process for building and submitting a new form.
