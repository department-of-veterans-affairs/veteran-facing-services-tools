...
* `CustomComponent`: A React element that will replace the default [`PrivacyAgreement`](https://github.com/department-of-veterans-affairs/vets-website/blob/master/src/platform/forms/components/PrivacyAgreement.jsx) inside the [SubmitController](https://github.com/department-of-veterans-affairs/vets-website/tree/master/src/platform/forms-system/src/js/review/SubmitController.jsx) component.
...
`sectionCompleted` is a function which expects a single boolean parameter indicating when all conditions of your `preSubmitInfo.CustomComponent` are met. Until `sectionCompleted` is called with `true`, the form will not submit.
