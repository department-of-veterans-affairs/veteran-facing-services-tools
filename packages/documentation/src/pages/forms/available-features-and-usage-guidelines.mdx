...
* If you don't specify a **PreSubmitSection**, no notice or checkbox appears above the submit button. Most applications will want to give some sort of notice to the user before they submit the form. Although this section is optional, we recommend you specify it.
...
See below for CustomComponent params

| Prop | Description | Required |
| -------- | ------ | ------ |
| formData | Data object from the rest of the form, so your CustomComponent can be reactive to external data | false |
| preSubmitInfo | Data object, See table above | false |
| showError | Boolean for your CustomComponent to toggle errors | false |
| sectionCompleted | Is a function, provided by `PreSubmitSection`, which expects a single boolean parameter indicating when all conditions of your `preSubmitInfo.CustomComponent` are met. Until `sectionCompleted` is called with `true`, the form will not submit. | true |
