...
- `itemName` - The name of the set of data in the card. This shows up as the title of the data set and may be in title case.
- `itemNameLowerCase` - The name of the set of data. This shows up as a `"New {itemNameLowerCase || itemName}"` button or `"Add additional {itemNameLowerCase || itemName}"`, only if `volatileData` is set to `true`. This is included in addition to `itemName` because converting title-case to lower case isn't always a simple process, e.g. abbreviations and certain words need to be capitalized ("Veteran").
