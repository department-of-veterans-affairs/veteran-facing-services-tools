...
- [Misc Field options](#misc-field-options)
...
- Only applies when setting a schema type to `array`.
- The `ArrayField` code will add an "Add Another" button with a suffix from `itemName` (usually in title-case)
- Including an `addAnotherLabel` replaces the add another button text:

  ```js
  names: {
    'ui:title': 'Bunch of names',
    'ui:options': {
      itemName: 'Name', // title case
      addAnotherLabel: 'Add another name' // sentence case
    }
  }
  ```

  Results in a title of "New Name" & a button containing "Add another name"
