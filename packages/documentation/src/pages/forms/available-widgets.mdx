...
Widgets are React components that return specific form elements. Set these widgets in a config file while building your form. The uiSchema `ui:widget` property tells the form which UI widget should be used to render a field.

Some widgets are associated with particular schema types or formats. There are additional widgets available, but the VA Forms Library uses the definitions they're included in rather than just the field.

Widgets from the [react-jsonschema-form library](https://react-jsonschema-form.readthedocs.io/en/latest/usage/widgets/) include string mappings. Widgets created specifically for the VA Forms Library do not have mappings, and therefore must be specified by passing the component for the widget directly to the config. To include such widgets, import the widget at the top of the file:
