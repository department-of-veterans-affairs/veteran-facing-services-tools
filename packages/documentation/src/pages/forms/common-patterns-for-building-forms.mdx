...
Note that `arrayPath: 'childrenToAdd'` is used. This is the name specified for the `array` inside the form `schema` objects, so we need to reference it here. The forms library will take care of connecting the dots as to what data goes with each item in the `array`.
