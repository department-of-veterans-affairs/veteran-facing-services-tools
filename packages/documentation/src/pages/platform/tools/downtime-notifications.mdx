...
- Create a maintenance window sometime in the future.
  - You can specify a message to a user by using the token "USER\_MESSAGE: " in the description (i.e. "Down for v16 update. USER\_MESSAGE: We'll be back soon!")
    - - Note that custom messages are discouraged to maintain a consistent user experience.
- Visit https://staging-platform-api.va.gov/v0/maintenance\_windows and ensure that your service's downtime window is listed
