...
The VA is currently in the process of moving content from several other systems into one primary content management system, powered by Drupal. VA.gov takes a decoupled approach, where the cms is queried during the VA.gov build process and content is transformed into static html and deployed on the site. Drupal does not control any Veteran-facing rendering.
...
Content displayed on VA.gov exists in three primary locations:
...
| production | http://prod.cms.va.gov | https://va.gov |

## How content is transformed into pages

VA.gov is a static html/js/css site, built with a static site generator tool called Metalsmith. During the Metalsmith build process we query the appropriate Drupal environment with a GraphQL query. The results of that query are transformed into pages through the Metalsmith build pipeline. There are liquid templates specific to page content that came from Drupal, generally denoted with a `.drupal.liquid` suffix. Each `entity` from Drupal has a corresponding `entityType`, which is used to choose the entry template for each `entity` (i.e. page) from Drupal.
