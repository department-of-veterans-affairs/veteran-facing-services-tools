...
Each pull request (PR) on `vets-website` runs through an automated process looking for code additions and modifications that don't meet code quality standards. If a potential issue is found, a bot will leave an automated comment and request a review from the **frontend-review-group** (see below).
