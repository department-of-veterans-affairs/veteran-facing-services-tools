...
- All connections made by VA.gov must be secure (HTTPS)
- VA.gov automatically redirects to a secure protocol if a non secure connection is attempted 

## Cross-Origin Resource Sharing (CORS)

- Some cross origin connections made by VA.gov must support CORS including **all** XHR connections and web font downloads. 
...
- All VA.gov pages must return a Content Security Policy header that allows only known scripts to run on the page
- The CSP must be *enforced* i.e. not set to `report-only`
- The `report-uri` should point to a VA platform error capturing service (Sentry)
