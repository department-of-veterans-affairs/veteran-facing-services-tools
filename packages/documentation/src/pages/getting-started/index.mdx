...
6. Clone VA.gov git repos **as sibling directories**:
   ...
   **Front end repos**
   ...
   - `content-build`: Metalsmith templates and content build for static pages
...
8a. Build `vets-website`. Make sure you [configured the SOCKS proxy](https://github.com/department-of-veterans-affairs/va.gov-team/blob/master/platform/engineering/internal-tools.md#configure-the-socks-proxy) to fetch content from Drupal.
   If you're only doing application work inside `vets-website` you can build with the scaffold option:
   ```bash
   $ yarn build:webpack --env.scaffold
   ```
...
10a. Open [http://localhost:3001](http://localhost:3001) in a browser
