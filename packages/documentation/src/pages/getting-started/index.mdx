...
in `content-build`, we recommend leaving this command running so you will be able to see the changes in `content-build`.
   ...
   If you would like webpack to open a browser window for you, please run `yarn watch --open`. We use [Webpack Devserver](https://webpack.js.org/configuration/dev-server/) to watch the files locally; all othe same ptions and documentation should apply.
