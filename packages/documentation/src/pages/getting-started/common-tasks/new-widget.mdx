...
2. Add a start up function to: `src/applications/static-pages/static-pages-entry.js`:
...
3. Write your create widget function:
...
Note that we don't import the widget code unless there's a mount point in the DOM. This functio will run on every page, so we want to make sure we only download the bulk of the widget code when we're actually going to use it.
