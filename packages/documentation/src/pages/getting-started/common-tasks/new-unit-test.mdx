...
- The `vets-website` repo uses:
  ...
  - [React Testing Library](https://testing-library.com/docs/react-testing-library/intro) and [Enzyme](https://airbnb.io/enzyme/) for mounting and inspecting React components
...
```
- Use `describe` to organize tests by application and feature. Two levels is adequate depending on the size of the application being tested. Try to avoid nesting deeper than two levels.
- Use `it` to describe the unit test:
  ...
  - Use RTL's [render](https://testing-library.com/docs/react-testing-library/api/#render) when testing components.
...
- If you're using Enzyme, you can use the other `schemaform-utils` for filling out form data:
...
```
We have successfully performed a "Happy Path" test for this component. Let's breakdown this test:
```js
// Setup
...
const handleSubmit = sinon.spy();
...
...
```
The setup for this test is self-explanatory. We create a mocked submit function and pass that to our form component, and use the [render](https://testing-library.com/docs/react-testing-library/api/#render) function to produce actual DOM nodes. Then we define user data for reuse later on.
**Note:** In this example we opt-in to destructuring the return value from our `render` function to give us our query functions. Optionally, you can utilize [screen](https://testing-library.com/docs/queries/about#screen) to access the query functions.
```js
// DOM Interaction and Queries
...
userEvent.type(getByLabelText(/username/i), user.username);
userEvent.type(getByLabelText(/password/i), user.password);
userEvent.click(getByText(/submit/i));
...
...
```
To conclude this test we need to check for two things:
- that our submit button was clicked and our onSubmit function fired
- that our onSubmit function fired with the correct data
...
- [React Testing Library](https://testing-library.com/docs/react-testing-library/intro): React Testing Library is a branch of `Testing Library` that allows users to test their components through actual DOM nodes instead of rendered instances.
