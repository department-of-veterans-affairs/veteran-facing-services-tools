...
tags: Nightwatch, mockapi, e2e
---

# Write an end to end test

Front end engineers use end-to-end tests in `vets-website` to validate multipage applications with client-side routing. They are primarily used to assert that:
...
- Two node apps run with the end to end tests:
  ...
  - `test-server.js` - builds and hosts `vets-website` with support for client-side routes
...
module.exports = E2eHelpers.createE2eTest(client => {
  // mock api responses
  MyApplicationHelpers.initApplicationSubmitMock();
  MyApplicationHelpers.initEnrollmentStatusMock();
...
- separate navigation from field input
  ...
  - use a **helper file** that for filling out forms
...
- mock all api responses before starting the test. See [Mocking API responses](#mockimg-api-responses)
...
- `mock-helpers.js` for mocking data
  - mock server is started as part of the end to end testing script
- mock all api calls in your application e.g. submit and upload calls
...
_`src/platform/testing/e2e` contains other useful helpers_
