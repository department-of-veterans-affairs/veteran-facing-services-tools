...
Most VA systems also connect to other systems (for example, My HealtheVet connects to VistA and EVSS connects to BGS and VBMS), creating a dependency tree. Much of that complexity is abstracted by API calls, but it can affect our infrastructure, especially when an upstream dependency goes down.
